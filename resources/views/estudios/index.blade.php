...
<li aria-current="page">
    <div class="flex items-center">
        ...
        <span class="ml-1 text-sm font-medium text-gray-900 md:ml-2 dark:text-white">Estudios</span>
    </div>
</li>
@endsection
<x-app-layout>
    <div>
        <div
            ...
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Estudios</h1>
                <a href="{{ route('estudios.create') }}" type="button"
                    ...
                    <tbody>
                        @forelse ($estudios as $estudio)
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    {{ $estudio->title }}</th>
                                <td class="px-4 py-3">{{ $estudio->institucion }}</td>
                                <td class="px-4 py-3">
                                    {{ Str::limit($estudio->descripcion, 25, '...') }}</td>
                                <td class="px-4 py-3">{{ $estudio->duracion }}</td>
                                <td class="px-4 py-3">{{ $estudio->fecha_desde_hasta }}</td>
                                <td class="px-4 py-3 ">{{ $estudio->actual ? 'Si' : 'No' }}</td>
                                <td class="px-4 py-3  ">
                                    ...
                                    <div id="apple-imac-27-dropdown"
                                        ...
                                        <div class="py-1">
                                            <a href="#"
                                                class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Borrar</a>
                                        </div>
                                    </div> --}}
                                    ...
                                    <div class="flex justify-center items-center">
                                        ...
                                        <button type="button" id="deleteModal-{{ $estudio->id }}"
                                            data-modal-toggle="deleteModal-{{ $estudio->id }}"
                            ...
                            {{-- Modal --}}
                            <div id="deleteModal-{{ $estudio->id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div
                                        ...
                                        <div class="flex justify-center items-center space-x-4">
                                            <form action="{{ route('estudios.destroy', $estudio->id) }}" method="POST"
                                                ...
                                                <button data-modal-toggle="deleteModal-{{ $estudio->id }}"
                                                    type="button"
                        ...
                        @empty
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 ">
                                <td class="text-center px-6 py-4" colspan="7">No hay estudios
                                    registrados aún. Crea uno nuevo <a href="{{ route('estudios.create') }}"
                                        class="text-blue-300">aquí</a></td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
