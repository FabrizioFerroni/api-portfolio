    ...
    <div>
        ...
        <div
            ...
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Editar estudio</h1>

            </div>
            <div class="relative overflow-x-auto  sm:rounded-lg">
                <form action="{{ route('estudios.update', $estudio->id) }}" method="POST" novalidate>
                    @csrf
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 px-4">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="descripcion" name="descripcion" rows="8"
                                ...
                                placeholder="Descripción del estudio">{{ old('descripcion') ?? $estudio->descripcion }}</textarea>
                            @error('descripcion')
                                <div class="text-red-500 pt-1 pl-0">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" name="fecha_desde_hasta" id="fecha_desde_hasta"
                                value="{{ old('fecha_desde_hasta') ?? $estudio->fecha_desde_hasta }}"
                            ...
                            <div class="flex flex-row gap-4">
                                <div class="flex flex-row items-center mb-4">
                                    <input id="si" type="radio" name="actual" value="1"
                                        ...
                                        @if (old('actual') || old('actual') === '1' || $estudio->actual === 1) checked @endif>
                                    <label for="si"
                                ...
                                <div class="flex flex-row items-center mb-4">
                                    <input id="no" type="radio" name="actual" value="0"
                                        class="w-4 h-4 border-gray-300 cursor-pointer dark:focus:bg-blue-600 dark:bg-gray-700 {{ $errors->has('actual') ? 'dark:border-red-700 border-red-700' : '' }} dark:border-gray-600" @if (old('actual') || old('actual') === 0 || $estudio->actual === 0) checked @endif>
                                    <label for="no"
                                        class="block ml-2 text-sm font-medium text-gray-900 cursor-pointer dark:text-gray-300"
                                        >
                                        No
