@section('title', 'Editar curriculum Vitae')
@section('breadcrumbs')
    ...
    <div>
        <div
            ...
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white px-4">Editar curriculum vitae
                </h1>

            </div>
            <div class="relative overflow-x-auto  sm:rounded-lg">
                {{-- aqui va el form --}}
                <form action="{{ route('cv.update', $cv->id) }}" novalidate method="POST"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 px-4">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre') ?? $cv->nombre }}"
                                ...
                                <div class="flex flex-row gap-4">
                                    <div class="flex flex-row items-center mb-4">
                                        <input id="si_online" type="radio" name="actual" value="1"
                                            ...
                                            @if (old('actual') || old('actual') === 1 || $cv->actual === 1) checked @endif>
                                        <label for="si_online"
                                    ...
                                    <div class="flex flex-row items-center mb-4">
                                        <input id="no_online" type="radio" name="actual" value="0"
                                            class="w-4 h-4 border-gray-300 cursor-pointer dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600 {{ $errors->has('actual') ? 'dark:border-red-700 border-red-700' : '' }}"
                                            checked @if (old('actual') || old('actual') === 0 || $cv->actual === 0) checked @endif>
                                        <label for="no_online"
                                            class="block ml-2 text-sm font-medium text-gray-900 cursor-pointer dark:text-gray-300">
                                            No
                                        </label>
